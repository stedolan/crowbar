...
let list g =
  let rec l = lazy (choose [
    const [];
    map [g; unlazy l] (fun x xs -> x :: xs)]) in
  unlazy l
...
let test = map [xml; xml] (fun xml1 xml2 ->
    (* Printf.printf "%s - %s\n%!" (Xmldiff.string_of_xml xml1) (Xmldiff.string_of_xml xml2); *)
    fun () ->
    let (patch, xml3) = Xmldiff.diff_with_final_tree xml1 xml2 in
    if xml2 <> xml3 then
      failwith (Xmldiff.string_of_xml xml2 ^ " != " ^ Xmldiff.string_of_xml xml3))

let () =
  Crowbar.Fuzz.fuzz test
